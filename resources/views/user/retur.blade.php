...
<div class="w-full py-8">
    ...
    <form method="POST" action="{{ route('user.profil.simpan') }}" class="w-1/2 mt-8 space-y-4">
        ...
        <div class="grid grid-cols-1 gap-2">
            <label for="no_resi" class="w-full mb-2 mt-2 text-sm font-medium ">No Resi Paket / No Transaksi</label>
            <input type="text" name="no_resi" id="no_resi" class="bg-white rounded border-gray-300 p-2.5 focus:outline-none focus:ring-0 focus:border-rose-400" placeholder="No Resi / No Transaksi">
            @error('no_resi')
        ...
        <div class="grid grid-cols-1 gap-2">
            ...
            <input type="file" name="bukti_resi" id="bukti_resi" class="bg-white rounded border-gray-300 p-2.5 focus:outline-none focus:ring-0 focus:border-rose-400">
            @error('bukti_resi')
                <div class="ml-20 text-sm text-red-500 italic">{{ $message }}</div>
            @enderror
        </div>
        <div class="grid grid-cols-1 gap-2">
            ...
            <input type="file" name="foto_produk" id="foto_produk" class="bg-white rounded border-gray-300 p-2.5 focus:outline-none focus:ring-0 focus:border-rose-400" placeholder="Password">
            @error('foto_produk')
                <div class="ml-20 text-sm text-red-500 italic">{{ $message }}</div>
            @enderror
        </div>
